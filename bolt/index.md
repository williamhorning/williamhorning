---
layout: home
title: home

hero:
  name: bolt
  text: a chatbot
  image:
    src: logo.svg
  actions:
    - theme: brand
      text: Users Guide
      link: https://williamhorning.eu.org/lightning/users
    - theme: alt
      text: Learn more
      link: https://williamhorning.eu.org/lightning/about
    - theme: alt
      text: View on Github
      link: https://github.com/williamhorning/bolt

features:
  - icon:
      src: community.svg
      alt: set of hands
    title: Connecting Communities
    details: Bolt bridges many popular messaging apps—such as Discord, Guilded, Revolt, and Telegram
  - icon:
      src: puzzle.svg
      alt: Puzzle Piece
    title: Extensible
    details: Bolt has many settings for users, allowing them to be enabled/disabled
  - icon:
      src: deno.svg
      alt: Deno
    title: Built on open-source technology
    details: Bolt is built on Lightning, which uses Deno and other open-source technology
---

### Invite the bot

- [Discord](https://williamhorning.eu.org/bolt/invite/discord)
- [Guilded](https://williamhorning.eu.org/bolt/invite/guilded)
- [Revolt](https://williamhorning.eu.org/bolt/invite/revolt)
- [Telegram](https://williamhorning.eu.org/bolt/invite/telegram)

### Get support

- [Discord](https://williamhorning.eu.org/bolt/support/discord)
- [Guilded](https://williamhorning.eu.org/bolt/support/guilded)
- [Revolt](https://williamhorning.eu.org/bolt/support/revolt)
- [Telegram](https://williamhorning.eu.org/bolt/support/telegram)
